...
} from '@polymer/paper-behaviors/paper-inky-focus-behavior.js';
...
const template = html`
  <style>
    ...
    .ring > .slider-knob > .slider-knob-inner {
      ...
      border: 2px solid
        var(--paper-slider-knob-start-border-color, var(--paper-grey-400));
    }
    ...
    .disabled > .slider-knob > .slider-knob-inner {
      ...
      border: 2px solid
        var(--paper-slider-disabled-knob-color, var(--paper-grey-400));
...
Polymer({
  ...
  _trackX: function(event) {
    ...
    var dx = Math.min(
      this._maxx,
      Math.max(this._minx, event.detail.dx * direction)
    );
    this._x = this._startx + dx;

    var immediateValue = this._calcStep(
  ...
  _allowBarEvent: function(event) {
    return (
      !this.ignoreBarTouch || event.detail.sourceEvent instanceof MouseEvent
    );
  },
  ...
  _leftKey: function(event) {
    if (this._isRTL) this._incrementKey(event);
    else this._decrementKey(event);
  },

  _rightKey: function(event) {
    if (this._isRTL) this._decrementKey(event);
    else this._incrementKey(event);
  },
